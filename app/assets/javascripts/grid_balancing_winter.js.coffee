class GridBalancingWinter
  ...
  setup: () ->
    ...
    target.append("<div id='winter_chart' class=''></div>")

    document.getElementById("pathway_box").style.display = "block"
    document.getElementById("winter_chart").style.display = "block"
    ...
    $('#display').append("<h5>Explore</h5><ul class='subnav'><li><a href='#' id='energy-subnav-1' class='btn btn-default' onclick='twentyfifty.switchView(\"grid_balancing_summer\")'>Typical Summer Day</a></li><li><a href='#' id='energy-subnav-2' class='btn btn-default' onclick='twentyfifty.switchView(\"grid_balancing_monsoon\")'>Typical Monsoon Day</a></li><li><a href='#' id='energy-subnav-3' class='btn btn-default selected' onclick='twentyfifty.switchView(\"grid_balancing_winter\")'>Typical Winter Day</a></li></ul>")
    
    #$('#warning').append("<h5>Warning</h5>")

    @winter_chart = new Highcharts.Chart({
      chart: { renderTo: 'winter_chart', height: 250},
      title: { text: 'Grid dispatch pattern for your chosen pathway', y:10 },
      subtitle: { text: 'Typical Winter Day (in hrs)' },
      yAxis: {
  ...
  updateResults: (@pathway) ->
    ...
    for name in titles
      data = @pathway['grid_balancing_winter'][name]
      if @winter_chart.series[i]?
        @winter_chart.series[i].setData(data,false)
      else
        @winter_chart.addSeries({name:name,data:data,color:color[i]},false)
      i++
...
window.twentyfifty.views['grid_balancing_winter'] = new GridBalancingWinter
