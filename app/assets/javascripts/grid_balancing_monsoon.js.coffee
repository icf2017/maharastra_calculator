class GridBalancingMonsoon
  ...
  setup: () ->
    ...
    target.append("<div id='monsoon_chart' class=''></div>")

    document.getElementById("pathway_box").style.display = "block"
    document.getElementById("monsoon_chart").style.display = "block"
    ...
    @monsoon_chart = new Highcharts.Chart({
      chart: { renderTo: 'monsoon_chart', height: 250},
      title: { text: 'Grid dispatch pattern for your chosen pathway', y:10 },
      subtitle: { text: 'Typical Monsoon Day (in hrs)' },
      yAxis: {
  ...
  updateResults: (@pathway) ->
    ...
    for name in titles
      data = @pathway['grid_balancing_monsoon'][name]
      if @monsoon_chart.series[i]?
        @monsoon_chart.series[i].setData(data,false)
      else
        @monsoon_chart.addSeries({name:name,data:data,color:color[i]},false)
      i++
...
##### Created an array ChartArr, optionsArr, chartIdArr to get the data of series , legand and chart id to create custom legand
    ...
    ChartArr = [
      @monsoon_chart.series
    ]

    optionsArr = [
      @monsoon_chart.options.legend
    ]
    chartIdArr = [
      "#monsoon_chart"
    ]
...
######### End mouse over and nouse out code ######################
    ...
    @monsoon_chart.redraw()    

    document.getElementById("warning").innerHTML="<p>"+@pathway['grid_msg1']+"<br><br>"+@pathway['grid_msg2']+"</p>"


window.twentyfifty.views['grid_balancing_monsoon'] = new GridBalancingMonsoon
