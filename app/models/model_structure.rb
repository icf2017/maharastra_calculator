...
class ModelStructure < ModelUtilities
  ...
  def types
    @types ||= (5..63).to_a.map { |row| excel.send("control_f#{row}") }
  end
  
  def names
    @names ||= (5..63).to_a.map { |row| excel.send("control_d#{row}") }
  end

  def descriptions
    @descriptions ||= (5..63).to_a.map { |row| [r("control_h#{row}"),r("control_i#{row}"),r("control_j#{row}"),r("control_k#{row}"),r("control_l#{row}"),r("control_m#{row}")] }
  end

  def long_descriptions
    @long_descriptions ||=  (5..63).to_a.map  { |row| [r("control_bo#{row}"), r("control_bp#{row}"),r("control_bq#{row}"),r("control_br#{row}")] }
  end
